...
* Amazon Elastic Block Store (Amazon EBS) to ___. <!---To do what? Do we need to mention the Container Storage Interface (CSI) storage driver in this high-level list? If so, what to say?---> 
* Amazon Relational Database Service (Amazon RDS) for PostgreSQL to ___. <!---To do what?--->
* Amazon Elastic Container Registry (Amazon ECR) to provide a private registry.
* AWS Key Management Service (AWS KMS) to ___. <!---To do what?-->
